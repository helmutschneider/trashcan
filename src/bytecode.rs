...
pub enum Instruction {
    ...
    ElementAddressOf(Rc<Variable>, Rc<Variable>, Argument),
    Return(Argument),
...
impl std::fmt::Display for Instruction {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ...
            Self::ElementAddressOf(dest_var, source, elem) => {
                format!("  elea {}, {}, {}", dest_var, source, elem)
            }
            Self::Return(value) => {
                format!("  ret {}", value)
            }
...
impl Bytecode {
            ...
            ast::Expression::BinaryExpr(bin_expr) => {
                ...
                match bin_expr.operator.kind {
                    TokenKind::Plus => {
                        ...
                        self.emit_copy(&dest_ref, lhs, stack);
                    ...
                    TokenKind::Equals => {
                        ...
                        if is_indirect {
                            ...
                            self.emit_copy(&lhs_var, rhs, stack);
                        }
                    }
            ...
            ast::Expression::UnaryPrefix(unary_expr) => {
                ...
                let dest_ref: Rc<Variable> = match unary_expr.operator.kind {
                    ...
                    TokenKind::Minus => {
                        ...
                        self.emit_copy(&neged_var, Argument::Int(0), stack);
                ...
                let init_var = match &init_arg {
                    Argument::Variable(x) => {
                        if expression_needs_explicit_copy(&var.initializer) {
                            ...
                            self.emit_copy(&var, init_arg, stack);
                            var
                        } else {
                            Rc::clone(x)
                        }
    ...
    fn emit_address_of(&mut self, var: &Rc<Variable>, stack: &mut Stack) -> Rc<Variable> {
        let type_ = Type::Pointer(Box::new(var.type_.clone()));
        let ptr_to_var = self.emit_variable(&type_, stack);
        self.emit(Instruction::AddressOf(Rc::clone(&ptr_to_var), Argument::Variable(Rc::clone(var))));
        return ptr_to_var;
    }
}
