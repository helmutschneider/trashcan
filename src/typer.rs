...
use crate::ast::{ASTLike, Function, Statement, StatementIndex, SymbolKind};
...
impl TypeTable {
    ...
    fn add_type(&mut self, name: &str, kind: TypeKind) -> TypeId {
        let id = TypeId(self.data.len() as i64);
        let type_ = Type {
            id: id,
            name: name.to_string(),
            kind: kind,
...
impl Typer {
    fn try_infer_type(&self, expr: &ast::Expression) -> Option<&Type> {
        return match expr {
            ...
            ast::Expression::FunctionCall(fx_call) => {
                ...
                return self.types.get_type_by_name(&fx.return_type);
            }
    ...
    fn check_expression(&self, expr: &ast::Expression, errors: &mut Vec<Error>) {
        match expr {
            ast::Expression::FunctionCall(fx_call) => {
                ...
                let fx = self.ast.get_function(&fx_call.name);
                self.maybe_report_missing_type(&fx_call.name, fx, ident_location, errors);

                if let Some(fx_sym) = fx {
    ...
    pub fn from_code(program: &str) -> Result<Self, Error> {
        ...
        let type_id_bool = types.add_type(TYPE_NAME_BOOL, TypeKind::Scalar);
        let type_id_int = types.add_type(TYPE_NAME_INT, TypeKind::Scalar);
        let type_id_ptr_to_byte = types.add_type(
            &format!("&{}", TYPE_NAME_BYTE),
            TypeKind::Pointer(type_id_void),
        );

        let mut string_fields: HashMap<String, TypeId> = HashMap::new();
        string_fields.insert("length".to_string(), type_id_int);
        string_fields.insert("data".to_string(), type_id_ptr_to_byte);
        let type_id_string = types.add_type(TYPE_NAME_STRING, TypeKind::Struct(string_fields));
